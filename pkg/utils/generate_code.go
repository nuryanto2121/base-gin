...
// 	if err := conn.WithContext(ctx).Model(models.TmpCode{}).Where("prefix = ?", t.Prefix).First(t).Error; err != nil {
...
// 		}
// 		t.Code = code
// 		if err := conn.WithContext(ctx).Model(models.TmpCode{}).Where("id=?", t.ID).Updates(t).Error; err != nil {
